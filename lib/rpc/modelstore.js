...
var UserSchema = Joi.object().keys({id: Joi.string().token()});
var NodeIdSchema = Joi.string().token();

module.exports.calculateSyncItems = function(user, nodeId, modelName, nodeManifest) {
  Joi.assert(user, UserSchema);
  Joi.assert(nodeId, NodeIdSchema);
...
module.exports.doSyncItems = function(user, nodeId, modelName, pushedItems, requestedObjectIds) {
  Joi.assert(user, UserSchema);
  Joi.assert(nodeId, NodeIdSchema);
  Joi.assert(modelName, Joi.string().valid(VALID_MODELS));
